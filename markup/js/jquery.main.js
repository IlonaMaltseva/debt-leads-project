...
jQuery(function () {
	...
	isElementExist(".testimonial-slider", initSlider);
...
function initSlider() {
	$(".testimonial-slider").each(function () {
		...
		const swiper = new Swiper(self, {
			...
			navigation: {
				nextEl: $(self).closest('.section-testimonial').find(".slider-btn-next").get(0),
				prevEl: $(self).closest('.section-testimonial').find(".slider-btn-prev").get(0),
			},
		});
	});
}
...
function initAccordion() {
	$('.accordion__item').each(function () {
		const $accordionTitle = $(this).find('.accordion__title');

		$accordionTitle.on('click', function (e) {
			const $opener = $(e.currentTarget);
			const $item = $opener.parent('.accordion__item');

			if ($item.hasClass('open')) {
				$item.removeClass('open');
			} else {
				$item.parent('.accordion').find('.accordion__item').removeClass('open');
				$item.addClass('open');
			}
		});
	});
...
function initSmartMenu() {
	var distanceBetweenMenuAndNav = $(".header").offset().top + $(".header").innerHeight() - $(".nav-drop__wrap").offset().top - $(".nav-drop__wrap").innerHeight();
	$(".menu").smartmenus({
